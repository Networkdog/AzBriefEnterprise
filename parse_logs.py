...
print("=== FOUNDRY SPECIALIST DETAILS ===")
for e in events:
    logger = e.get("logger", "")
    event = e.get("event", "")
    # Check if this info belongs to the Foundry backend or specialist team.
...
for e in events:
    if "tool" in e:
        agent = e.get("agent")
        role = e.get("role") or e.get("stage")
        tool = e.get("tool")
        tool_round = e.get("tool_round")
        print(f"Agent: {agent}, Role: {role}, Tool: {tool}, Round: {tool_round}")
...
validation_error_count = sum(
    1
    for e in events
    if "validation-error" in str(e).lower() or "validation_error" in str(e).lower()
)
print("validation-error:", validation_error_count)

llm_repair_count = sum(
    1 for e in events if "llm-repair" in str(e).lower() or "llm_repair" in str(e).lower()
)
print("LLM-repair:", llm_repair_count)

round_limit_count = sum(
    1
    for e in events
    if "round" in str(e).lower() and ("limit" in str(e).lower() or "exceeded" in str(e).lower())
)
...
unknown_evidence_count = sum(
    1
    for e in events
    if "unknown-evidence" in str(e).lower() or "unknown_evidence" in str(e).lower()
)
print("unknown-evidence:", unknown_evidence_count)

review_events_count = sum(
    1
    for e in events
    if e.get("agent") == "azbrief-quality-reviewer"
    or e.get("event") in {"critic_rewrite_started", "critic_rewrite_done"}
)
print("quality-review events:", review_events_count)
