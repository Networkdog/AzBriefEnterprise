...
- **편집형 이메일 레이아웃** — 순백색 바탕과 지면에 잉크색 `#182b32`, 청록색 `#08746b`를
  ...
  전체 폭 청록색 장 구분대로 위계를 나눕니다. 데스크톱 본문은 제목 18%·내용 82% 그리드와
  18.75px 섹션 제목·핵심 문장을 사용합니다. 섹션 제목 굵기는 525, 핵심 문장은 700입니다.
  좁은 화면과 CSS 제거 상태에서는 세로로 쌓습니다. 모바일 media query는 문서 제목을 29px,
  섹션 제목·핵심 문장을 15.75px로 줄이며, inline-only 출력은 기본 크기를 유지합니다.
  Inline-only/MSO에서는 전체 너비 제목 아래
  평가축 이름과 값을 놓고, media query 지원 데스크톱에서는 제목과 3축을 나란히 표시합니다.
  등급 배지는 18px 글자를 사용하며 inline-only에서도 공간이 부족하면 평가축 이름과 배지가
