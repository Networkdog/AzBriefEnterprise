...
- **공통 위계**: 단건과 digest 상세가 공통 header/section formatter를 사용. Section 제목·핵심 문장은
     데스크톱 18.75px·모바일 15.75px이며 굵기는 각각 525·700. 본문은 13px·주요 블록 행간
     1.8~1.85. `cover=36`, `stat=48`로 발행물 이름·문서 제목·수치를 구분하고 모바일 제목은
     29px로 표시. 본문은 데스크톱 제목 18%·내용 82%, 목차는 전체 너비로 구성
- **상태는 텍스트와 색상으로 표현**: 중요성·영향도·직무연관성을 합치지 않고 각 높음/보통/낮음
     label을 18px 배지로 유지. 공간이 부족하면 평가축 이름과 배지를 함께 줄바꿈하며 글자를
