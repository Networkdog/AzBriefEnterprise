...
| 57번 Company Data Sheet의 정보 열 | 데스크톱 제목 15%·본문 85% 그리드와 두 줄 제목, 목차는 전체 너비, 좁은 화면은 세로 배치 |
...
- `FONT_SIZE_PX`의 본문 13px 단계는 유지하고 `cover=36`, `stat=48`을 표시용으로 추가합니다.
  발행물 이름은 36px, 문서 제목은 48px이며 모바일에서는 모두 29px입니다. 공통 section 제목은
  데스크톱 20.625px·모바일 17.325px·굵기 525입니다. 핵심 문장은 18.75px·15.75px·굵기 700을 유지합니다.
...
- `format_email_section_html()`은 화면 800px 이상에서 제목 15%·본문 85% 그리드로 배치하며
  여러 어절인 제목은 첫 어절 뒤에서 줄바꿈합니다. `count_text`로 받은 리소스 건수는 11px 보조
  글자로 구분합니다. 모바일·inline/MSO에서는 전체 너비 표를 쌓고 제목을 이어 씁니다. `full_width=True`인 목차는 전체 너비를
...
- 등급·검증 배지, 핵심 요약, concept box, 추가 확인의 세로 강조선에는 공통
  ...
  padding은 각각 4px를 유지하고 등급 배지 글자는 13.5px를 사용합니다. 높이 0·비표시·aria-hidden인
  18px 이름표로 기존 너비를 유지하고 행 높이 27px로 박스 높이도 유지합니다. 각 평가축 이름·배지는
