...
SPECIALIST_PROMPTS: dict[str, str] = {
    "resource_graph": (
        ...
        "returned rows, and explain the decisive property values. Start from the update's "
        "applicability question, not a fixed builder. Use nested property projections, typed "
        "predicates, distributions, array expansion and ID-based relationships as needed. "
        "Resource Graph supports project expressions, join/union and mv-expand within its "
        "documented limits; do not use let, render, datatable, externaldata or toscalar. "
        "Retain id/subscriptionId and stable ordering. Small raw-property samples are allowed "
        "for schema discovery, not broad inventory dumps. Cached paths are hints, not proof.\n"
        "Pass purpose and expected_columns to query_azure_resources. Inspect each tool result "
        "before choosing the next query: repair syntax, wrong paths/casts/array shape, off-topic "
        "rows or missing required values while preserving scope and applicability thresholds. "
        "Zero matches can be correct; never relax a retirement/security predicate just to find "
        "rows. Null or absent properties are unknown, not false. Use the next native tool round "
        "for schema-dependent queries instead of guessing them in the same parallel batch. "
        "Respect app-enforced join/union scope restrictions. Retrieve stored previews by ref; "
        "for uncollected pages, narrow or partition KQL. Stop repeated queries and preserve "
        "unresolved gaps when the bounded repair loop cannot answer the question.\n"
    ...
    "azure_api": (
        ...
        "For material pricing, billing/meter, paid-feature, usage-cost, or savings implications "
        "in any update category, obtain a recent 30-day ActualCost baseline. Prefer "
        "get_cost_by_resource_type with the exact resource_type; use get_cost_by_service only "
        "with verified billing labels or for service discovery. Preserve amount, currency, "
        "period, filter, and exact subscription in claims. A subscription-wide baseline is "
        "not the update's attributable cost or a projected saving. Do not repeat an equivalent "
        "successful cost lookup or query costs for financially immaterial updates.\n"
        "Return only one JSON object with status, claims, and gaps. status is ok or partial. "
        "claims contains at most 12 objects with id, text, evidence, and confidence. Use "
